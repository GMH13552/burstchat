"""
DeepSeek API 客户端: 调用 LLM 生成带时间戳的消息序列
"""
...
class LLMClient:
    def __init__(self, api_key: str, persona: str = "xiaoye", model: str = "deepseek-chat"):
        ...
        self._system_prompt = self.persona.build_system_prompt()
    ...
    async def plan_messages(
        ...
        self._debug_log(
            ...
            f"last_msgs={json.dumps([m['content'][:50] for m in messages[-4:]], ensure_ascii=False)}"
        )
